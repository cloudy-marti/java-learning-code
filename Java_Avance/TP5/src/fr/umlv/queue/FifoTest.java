...
public class FifoTest {
    ...
    public void shouldGetAnErrorWhenOfferingToFullFifo() {
        var fifo = new fr.umlv.queue.Fifo<Integer>(1);
        fifo.offer(43);
    ...
    public void shouldGetAnErrorWhenOfferingNull() {
        var fifo = new fr.umlv.queue.Fifo<>(234);
    ...
    public void shouldGetOfferedValueWhenPollingWithMixedTypes() {
        var fifo = new fr.umlv.queue.Fifo<>(20);
        for (var i = 0; i < 20; i++) {
            fifo.offer(i);
        }
    ...
    public void shouldBeAbleToAddMoreThanCapacityAfterRemoval() {
        var fifo = new fr.umlv.queue.Fifo<String>(2);
        fifo.offer("foo");
        fifo.poll();
        fifo.offer("1");
        fifo.offer("2");
    ...
    public void shouldPrintEmptyFifo() {
        var fifo = new fr.umlv.queue.Fifo<>(23);
        assertEquals("[]", fifo.toString());
    }

    @Test
    public void shouldPrintFifoWithOneElement() {
        var fifo = new fr.umlv.queue.Fifo<String>(23);
        fifo.offer("joe");
    ...
    public void shouldPrintFifoWithTwoElements() {
        var fifo = new fr.umlv.queue.Fifo<Integer>(23);
        fifo.offer(1456);
        fifo.offer(8390);
    ...
    public void shouldNotAffectFifoWhenPrinting() {
        var fifo = new fr.umlv.queue.Fifo<Integer>(200);
    ...
    public void shouldPrintFifoInTheSameWayAsAList() {
        var fifo = new fr.umlv.queue.Fifo<Integer>(99);
        var list = new ArrayList<Integer>();
        for (var i = 0; i < 99; i++) {
            fifo.offer(i);
            list.add(i);
    ...
    public void shouldAnswerOneWhenAskedForTheSizeAfterOneOffer() {
        var fifo = new fr.umlv.queue.Fifo<String>(1);
        fifo.offer("dooh");
        assertEquals(1, fifo.size());
    }
    ...
    public void shouldNotGetSideEffectsWhenUsingIteratorHasNext() {
        var fifo = new fr.umlv.queue.Fifo<Integer>(3);
        fifo.offer(117);
        fifo.offer(440);
        var it = fifo.iterator();
    ...
    public void shouldIterateOverALargeNumberOfElements() {
        var fifo = new fr.umlv.queue.Fifo<Integer>(10_000);
  ...
  public void shouldNotGetAnOverflowErrorWhenIteratingOverAnAlmostMaximalCapacityFifo() {
    var fifo = new fr.umlv.queue.Fifo<Integer>(Integer.MAX_VALUE - 8);
    ...
    public void shouldBeAbleToUseImplicitForEachLoop() {
        var fifo = new fr.umlv.queue.Fifo<Integer>(100);
        fifo.offer(222);
        fifo.poll();
